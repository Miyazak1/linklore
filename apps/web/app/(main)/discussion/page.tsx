...
export default function UploadPage() {
	...
	const [topicType, setTopicType] = useState<'article' | 'discussion'>('discussion');
				...
				<form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-lg)' }}>
					...
					<div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-sm)' }}>
						<label style={{ 
							...
							话题类型 <span style={{ color: 'var(--color-error)', fontSize: 'var(--font-size-lg)' }}>*</span>
						</label>
						<div style={{ display: 'flex', gap: 'var(--spacing-md)' }}>
							<label style={{
								...
								border: `2px solid ${topicType === 'article' ? 'var(--color-primary)' : 'var(--color-border)'}`,
								borderRadius: 'var(--radius-md)',
								background: topicType === 'article' ? 'var(--color-primary-lighter)' : 'var(--color-background)',
								transition: 'all var(--transition-fast)',
								opacity: uploading ? 0.6 : 1
							}}
							onMouseEnter={(e) => {
								if (!uploading && topicType !== 'article') {
									e.currentTarget.style.borderColor = 'var(--color-primary)';
							...
							>
								<input
									...
									name="topicType"
									value="article"
									checked={topicType === 'article'}
									onChange={(e) => setTopicType(e.target.value as 'article' | 'discussion')}
									disabled={uploading}
									style={{ 
							...
							<label style={{
								...
								cursor: uploading ? 'not-allowed' : 'pointer',
								padding: 'var(--spacing-md)',
								border: `2px solid ${topicType === 'discussion' ? 'var(--color-primary)' : 'var(--color-border)'}`,
								borderRadius: 'var(--radius-md)',
								background: topicType === 'discussion' ? 'var(--color-primary-lighter)' : 'var(--color-background)',
								transition: 'all var(--transition-fast)',
								opacity: uploading ? 0.6 : 1
							}}
							onMouseEnter={(e) => {
								if (!uploading && topicType !== 'discussion') {
									e.currentTarget.style.borderColor = 'var(--color-primary)';
									e.currentTarget.style.background = 'var(--color-primary-lighter)';
								}
							}}
