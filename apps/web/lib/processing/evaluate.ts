 ...
 */
function buildFewShotExamples(dimensions: string[], hasCitations: boolean): string {
	const examples: string[] = [];
	
	// Example 1: 高质量文档（有引用）
	if (hasCitations) {
		const example1: Record<string, any> = {
			scores: {},
			reasoning: {}
		};
	...
	const example2: Record<string, any> = {
		scores: {},
		reasoning: {}
	};
...
export async function evaluateAndStore(documentId: string) {
	// 检查依赖
	...
	const criteriaDetails: Record<string, string> = {
		...
		try {
			...
			if (evaluatedDocs.length >= 2) {
				...
				try {
					...
					// 找出包含当前文档作者的用户对
					const newPairs = userPairs.filter(pair => 
						pair.userId1 === doc.authorId || pair.userId2 === doc.authorId
					);
