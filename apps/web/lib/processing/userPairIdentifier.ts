...
export interface UserPair {
	...
	discussionPaths: Array<{
		path: string[];
		depth: number;
		direction: 'user1->user2' | 'user2->user1';
	}>;
}
 ...
 * 获取用户对在话题中的所有讨论文档
 */
export async function getUserPairDocuments(
	topicId: string,
	userId1: string,
	userId2: string
): Promise<Array<{ id: string; authorId: string; parentId: string | null }>> {
	...
	const relevantDocs: typeof userDocs = [];
	...
	userDocs.forEach(doc => {
		if (doc.parentId) {
			...
			if (trueOriginalDoc && doc.id === trueOriginalDoc.id) {
				...
				// 查找所有直接回复这个原始文档的文档
				const repliesToOriginal = userDocs.filter(d => 
					d.parentId === doc.id && 
