 ...
 * 计算内容hash
 */
export function calculateContentHash(body: string, citations: any[]): string {
 ...
 * 检查是否有缓存的分析结果
 */
export async function getCachedAnalysis(traceId: string, contentHash: string) {
	...
	if (currentHash === contentHash) {
		...
		if (analysis) {
			// 验证内容是否真的匹配
			const analysisHash = calculateContentHash(
				analysis.trace.body,
				analysis.trace.citationsList
			);
 ...
 * 保存分析结果（包含hash）
 */
export async function saveAnalysisWithHash(
	traceId: string,
	analysisData: any,
	contentHash: string
) {
