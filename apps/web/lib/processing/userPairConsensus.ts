...
export interface UserPairConsensusResult {
	...
	disagreements: Array<{
		...
		doc2Id: string;
		description?: string;
		similarity?: number; // 语义相似度（分歧点的相似度应该较低）
	}>;
 ...
 * 分析两个用户之间的共识和分歧
 */
export async function calculateUserPairConsensus(
	topicId: string,
	userId1: string,
	userId2: string
	...
	user1Docs.forEach(doc => {
		...
		if (summary && Array.isArray(summary.claims)) {
			summary.claims.forEach((claim: any) => {
...
请仔细分析观点的语义，不要只看关键词，要理解观点的实际含义。`;
	...
	let aiResult: {
		...
		disagreements: Array<{ claim1: string; claim2: string; doc1Id: string; doc2Id: string; description?: string }>;
	};
	...
	// 6. 使用语义相似度验证和增强共识点
	const enhancedConsensus = await Promise.all(
		(aiResult.consensus || []).map(async (item) => {
				...
				if (doc1 && doc2) {
					...
					if (summary1 && summary2) {
						...
						const claims1 = Array.isArray(summary1.claims) 
							? summary1.claims.filter((c: any): c is string => typeof c === 'string').join(' ')
							: '';
						const claims2 = Array.isArray(summary2.claims)
							? summary2.claims.filter((c: any): c is string => typeof c === 'string').join(' ')
							: '';
	...
	// 7. 使用语义相似度验证分歧点
	const enhancedDisagreements = await Promise.all(
		(aiResult.disagreements || []).map(async (item) => {
			// 分歧点的相似度应该较低
 ...
 * 保存用户对共识分析结果到数据库
 */
export async function saveUserPairConsensus(
	topicId: string,
	userId1: string,
	userId2: string,
	result: UserPairConsensusResult,
	docIds: string[],
	discussionPaths: Array<{ path: string[]; depth: number; direction: string }>
): Promise<void> {
	...
	await prisma.userConsensus.upsert({
		...
		update: {
			...
			disagreements: result.disagreements as any,
			consensusScore: result.consensusScore,
			divergenceScore: result.divergenceScore,
			docIds,
			discussionPaths: discussionPaths as any,
