...
export interface QualityCheckResult {
	...
	overallScore: number;
	criticalScore: number; // 关键维度平均分
	reasons: string[];
	suggestions: string[];
}

// 关键维度（用于判断是否能提炼有效观点）
const CRITICAL_DIMENSIONS: Record<string, string[]> = {
	default: ['观点', '逻辑', '证据'],
	'哲学': ['观点', '逻辑', '论证'],
	'文学': ['观点', '表达', '材料'],
	'历史': ['观点', '逻辑', '史料'],
 ...
 * 计算加权平均分
 */
function calculateWeightedScore(scores: Record<string, number>, discipline: string = 'default'): number {
	...
	Object.entries(scores).forEach(([key, value]) => {
		if (key === '_reasoning') return; // 跳过推理字段
 ...
 * 检查文档质量是否足够参与分歧分析
 */
export function checkDocumentQuality(
	...
	const hasBasicQuality = Object.entries(scores).some(([key, value]) => {
		if (key === '_reasoning') return false;
