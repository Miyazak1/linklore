...
const log = createModuleLogger('Reality Validator');
 ...
 * "感性的、对象性的、现实的活动"
 */
export async function validatePracticeReality(
  content: string,
  structuredData?: any,
...
`;
  ...
  try {
    ...
    // 计算综合分数
    let score = 0;
    if (result.hasConcreteAction) score += 0.33;
    if (result.hasExternalImpact) score += 0.33;
    if (result.hasMaterialResult) score += 0.34;
    ...
    log.error('validatePracticeReality failed', error as Error);
    // 返回默认值
    return {
