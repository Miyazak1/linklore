...
export interface ModerationResult {
	...
	details: {
		// 主题相关
		topicDrift?: string; // 话题偏离说明（第5条）
		topicUnclear?: string; // 主题不明确说明（第4条）
		
		// 事实与前提相关
		premiseError?: string; // 前提错误说明（第7条）
		premiseUnclear?: string; // 前提不明确说明（第7条）
		factSpeculationConfusion?: string; // 事实与推测混淆说明（第6条）
		
		// 推理相关
		logicalFallacies?: string[]; // 逻辑谬误列表（第8条）
		reasoningChainBreak?: string; // 推理链条断裂说明（第8条）
		
		// 表达方式相关
		emotionalExpression?: string; // 情绪化表达说明（第9条）
		emotionalEscalation?: string; // 情绪升级预警（第10条）
		disrespectfulContent?: string; // 不尊重内容说明（第9条）
		
		// 分歧处理相关
		disagreementType?: string; // 分歧类型（第11条）：事实层/概念定义/推理链/价值取向
		consensusConflict?: string; // 与已锁定共识冲突说明（第12条）
		
		// AI回答相关（如果是AI生成内容）
		aiFactualError?: string; // AI事实错误（第14条）
		aiEmotionalTone?: string; // AI情绪化语气（第15条）
		aiValueJudgment?: string; // AI价值裁决（第16条）
		aiConsensusBlocking?: string; // AI阻碍共识（第17条）
...
**第一章：总目标与基本原则**
- 讨论目的：保持主题聚焦、澄清事实与前提、促进理解与协调、最终达成可共同接受的共识
- 合作精神：讨论是合作性活动，非竞争或攻击
- AI从属性：AI必须服务于讨论目的，不能代替用户做最终判断

**第二章：讨论前提与主题管理**
- 第4条：主题必须明确；未定义、模糊或多重时，应提示澄清
- 第5条：主题偏离判定标准：
  * 与当前核心问题无逻辑关联
  * 引入无根据的旁枝观点
  * 将争论转向他人动机、身份、情绪等非议题内容

**第三章：事实、前提与推理**
- 第6条：必须区分"已确认的事实"、"未确认的推测"、"价值判断与个人偏好"
- 第7条：前提明确义务 - 必须说明观点依赖的前提，禁止在不明前提上继续推论
- 第8条：推理链条要求 - 禁止偷换概念、跳步推理、以偏概全、稻草人论证、情绪替代推理

**第四章：表达方式与情绪管理**
- 第9条：理性表达义务 - 观点清晰、不得侮辱/讽刺/贬低、不得以情绪替代事实、不得攻击性假设
- 第10条：情绪升级预警 - 当出现指责/归罪/标签化、情绪负荷增加、目的转为证明对方错误时，必须启动"降级模式"

**第五章：分歧处理与共识机制**
- 第11条：分歧拆分原则 - 必须拆分为：事实层分歧、概念定义分歧、推理链分歧、价值取向分歧
- 第12条：阶段性共识锁定 - 双方确认的事实/定义/前提必须被记录；推翻锁定共识必须先提供理由
- 第13条：可分歧结论原则 - 允许形成"明确标注的分歧点"，不强求绝对一致

${isAiGenerated ? `**第六章：AI辅助回答规范（本条消息是AI生成，需严格检查）**
- 第14条：AI不得制造虚假事实 - 不得虚构事实、编造数据、未标注的不确定推论
- 第15条：AI不得情绪化 - 语气必须中立、冷静、建设性
- 第16条：AI不得参与价值裁决 - 不得对用户谁"更正确"做裁定
- 第17条：AI必须促进共识形成 - 必须提高清晰度、降低冲突、增强可理解性、促进问题收敛` : ''}

**第七章：监督AI的权限与责任**
- 第18条：监督权 - 可标注逻辑错误、指出主题偏离、拦截不符合宪章的AI回答、降级情绪对话、要求澄清前提、要求结构化表达、协助记录共识点与分歧点
- 第19条：公正中立原则 - 无立场偏向、无情绪态度、不得协助任何一方"赢得讨论"、仅就"表达与逻辑质量"进行监督
- 第20条：最低干预原则 - 仅在偏离发生时介入，不改变双方讨论意图，仅恢复秩序
...
${isAiGenerated ? `9. **AI回答规范检查**（第14-17条，仅对AI生成内容）
   - 检查是否制造虚假事实
   - 检查是否情绪化
   - 检查是否参与价值裁决
   - 检查是否阻碍共识形成` : ''}
...
**请用JSON格式返回分析结果**：
{
  ...
  "details": {
    "topicDrift": "话题偏离说明（第5条，如果有偏离）",
    "topicUnclear": "主题不明确说明（第4条，如果主题模糊）",
    "premiseError": "前提错误说明（第7条，如果前提错误）",
    "premiseUnclear": "前提不明确说明（第7条，如果前提未明）",
    "factSpeculationConfusion": "事实与推测混淆说明（第6条，如果混淆）",
    "logicalFallacies": ["逻辑谬误1（第8条）", "逻辑谬误2"],
    "reasoningChainBreak": "推理链条断裂说明（第8条，如果断裂）",
    "emotionalExpression": "情绪化表达说明（第9条，如果存在）",
    "emotionalEscalation": "情绪升级预警（第10条，如果检测到升级）",
    "disrespectfulContent": "不尊重内容说明（第9条，如果存在）",
    "disagreementType": "分歧类型（第11条）：事实层/概念定义/推理链/价值取向",
    "consensusConflict": "与已锁定共识冲突说明（第12条，如果冲突）",
    ${isAiGenerated ? `"aiFactualError": "AI事实错误（第14条，如果存在）",
    "aiEmotionalTone": "AI情绪化语气（第15条，如果存在）",
    "aiValueJudgment": "AI价值裁决（第16条，如果存在）",
    "aiConsensusBlocking": "AI阻碍共识（第17条，如果存在）",` : ''}
...
- 对于话题偏离，要指出偏离的原因，并建议如何回到核心话题
- 对于AI生成内容，必须严格按照第14-17条检查
- 必须保持公正中立，不得偏向任何一方`;
