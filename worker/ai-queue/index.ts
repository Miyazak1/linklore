...
function initRedisConnection(): IORedis | null {
...
async function processExtract(job: any) {
...
async function processEvaluate(job: any) {
	const { documentId } = job.data as { documentId: string };
	// Dynamically import to reuse web lib code if available
...
async function processAnalyzeDisagreements(job: any) {
	const { topicId, newDocumentId } = job.data as { topicId: string; newDocumentId?: string };
...
async function processTrackConsensus(job: any) {
	const { topicId } = job.data as { topicId: string };
	// 使用新的用户对共识聚合逻辑
...
async function processUserPairAnalysis(job: any) {
	const { topicId, userId1, userId2 } = job.data as { topicId: string; userId1?: string; userId2?: string };
...
async function processChatAnalysis(job: any) {
	...
	try {
		const worker = new Worker(
			...
			async (job) => {
				...
				try {
					if (job.name === 'extract') return await processExtract(job);
					if (job.name === 'summarize') return await processSummarize(job);
					if (job.name === 'evaluate') return await processEvaluate(job);
...
export function addJob<T>(name: 'extract' | 'summarize' | 'evaluate', data: T, opts?: JobsOptions) {
	if (!queue) {
