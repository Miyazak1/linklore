 ...
 * 增强版：包含现实性分数和验证状态
 */
export function calculateCompositeScore(depthScore, densityScore, createdAt, likeCount = 0, commentCount = 0, realityScore = 0.5, // 新增：现实性分数（0-1）
isVerified = false // 新增：是否已验证
) {
    // 权重配置（调整后）
    ...
    const interactionScore = Math.min((likeCount * 0.6 + commentCount * 0.4) / 50, 1);
    // 时间衰减（越新越好，但权重较低）
    // 使用指数衰减，30天半衰期
    ...
    // 综合分数
    const compositeScore = depthScore * depthWeight +
 ...
 */
export async function updateCompositeScores(practiceIds) {
