...
function decodeSecret(encrypted) {
...
async function callOpenAiCompatible(endpoint, options) {
...
export async function callSiliconFlow(options, customEndpoint) {
...
export async function callAiProvider(provider, options) {
    switch (provider) {
        case 'openai':
...
export function getApiKeyFromConfig(encApiKey) {
    return decodeSecret(encApiKey);
}
/**
 * 流式调用 OpenAI-compatible API
 */
export async function callOpenAiCompatibleStream(endpoint, options) {
    // Ensure endpoint ends with /v1 or add it
    let apiUrl = endpoint;
    if (!apiUrl.endsWith('/v1') && !apiUrl.endsWith('/v1/')) {
        apiUrl = apiUrl.replace(/\/$/, '') + '/v1';
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(error?.error?.message || `API 调用失败: ${response.status} ${response.statusText}`);
        }
 ...
 * 解析流式响应
 */
export async function* parseStreamResponse(stream) {
    const reader = stream.getReader();
    const decoder = new TextDecoder();
    let buffer = '';
    let chunkCount = 0;
    let totalText = '';
    let savedUsage = null; // 保存usage，等收到content后再返回
            ...
            for (const line of lines) {
                ...
                if (line.startsWith('data: ')) {
                    ...
                    try {
                        ...
                        log.error('JSON解析错误', e, { data: data.substring(0, 200) });
                        // 忽略解析错误（可能是空行或其他格式）
                    }
                }
            }
