 ...
 * @returns 监督分析结果
 */
export async function moderateMessage(messageId, roomId) {
    ...
    try {
        ...
        await prisma.chatMessage.update({
            ...
            data: {
                ...
                moderationDetails: result.details
            }
        });
        // 更新房间的分析统计
 ...
 * 更新房间的分析统计
 */
async function updateRoomAnalysisStats(roomId, result, senderId) {
    try {
        // 获取或创建分析记录
        ...
        const analysis = await prisma.chatAnalysis.upsert({
            ...
            update: {
                ...
                creatorWarnings: result.status === 'WARNING' && senderId === room.creatorId
                    ? { increment: 1 }
                    : undefined,
                participantWarnings: result.status === 'WARNING' && senderId === room.participantId
                    ? { increment: 1 }
                    : undefined,
            ...
            create: {
                ...
                creatorWarnings: result.status === 'WARNING' && senderId === room.creatorId ? 1 : 0,
                participantWarnings: result.status === 'WARNING' && senderId === room.participantId ? 1 : 0,
                lastAnalyzedAt: new Date()
            }
        });
    }
