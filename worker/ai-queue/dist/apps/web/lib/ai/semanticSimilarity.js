...
import { getApiKeyFromConfig } from './adapters';
 ...
 * 2. 如果不支持，使用AI生成相似度评分
 */
export async function calculateSemanticSimilarity(text1, text2, provider, apiKey, apiEndpoint) {
    // 1. 检查缓存
    ...
    if (!provider || !apiKey) {
        ...
        if (systemConfig) {
            provider = systemConfig.provider || 'siliconflow';
 ...
 */
async function calculateSimilarityByEmbedding(text1, text2, provider, apiKey, apiEndpoint) {
    // 确定embedding模型
 ...
 * 使用AI生成相似度评分（备选方案）
 */
async function calculateSimilarityByAI(text1, text2, provider, apiKey, apiEndpoint) {
    // 如果没有provider，使用系统配置
    if (!provider || !apiKey) {
        const systemConfig = await prisma.systemAiConfig.findFirst({
            orderBy: { updatedAt: 'desc' }
 ...
 * 生成缓存键
 */
function getCacheKey(text1, text2) {
 ...
 * 批量计算语义相似度（优化性能）
 */
export async function calculateBatchSimilarity(textPairs, provider, apiKey, apiEndpoint) {
    // 并行计算，但限制并发数
    const batchSize = 5;
    const results = [];
    for (let i = 0; i < textPairs.length; i += batchSize) {
        const batch = textPairs.slice(i, i + batchSize);
        const batchResults = await Promise.all(batch.map(pair => calculateSemanticSimilarity(pair.text1, pair.text2, provider, apiKey, apiEndpoint)));
