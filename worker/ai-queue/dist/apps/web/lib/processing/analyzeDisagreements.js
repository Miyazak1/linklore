 ...
 */
export async function analyzeDisagreementsIncremental(topicId, newDocumentId) {
    // 检查缓存
...
async function performAnalysis(topicId, newDocumentId) {
    ...
    for (let i = 0; i < docPairs.length; i += batchSize) {
        ...
        const batchResults = await Promise.all(batch.map(([doc1, doc2]) => analyzePair(doc1, doc2, topicId)));
        allDisagreements.push(...batchResults.filter(d => d !== null));
    }
    // 去重
...
async function analyzePair(doc1, doc2, topicId) {
    const summary1 = doc1.summaries[0];
    const summary2 = doc2.summaries[0];
    if (!summary1 || !summary2) {
        return null; // 没有总结的文档不分析
    }
    const claims1 = Array.isArray(summary1.claims) ? summary1.claims.filter((c) => typeof c === 'string') : [];
    const claims2 = Array.isArray(summary2.claims) ? summary2.claims.filter((c) => typeof c === 'string') : [];
 ...
 * 去重分歧点
 */
function deduplicateDisagreements(disagreements) {
    const seen = new Set();
    const result = [];
 ...
 * 保存分歧点到数据库
 */
async function saveDisagreements(topicId, disagreements) {
    // 获取现有分歧点
    ...
    // 生成现有分歧点的hash
    const existingHashes = new Set(existing.map(d => {
        const key = `${d.doc1Id || ''}-${d.doc2Id || ''}-${d.title}`;
 ...
 */
export async function validateDisagreements(topicId) {
    ...
    for (const d of disagreements) {
        ...
        const docIds = Array.isArray(d.docIds) ? d.docIds : [];
        if (docIds.length === 0)
            continue;
        const docs = await prisma.document.findMany({
            where: { id: { in: docIds } },
