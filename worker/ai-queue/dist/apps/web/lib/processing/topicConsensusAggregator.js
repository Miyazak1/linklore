 ...
 * 基于所有用户对的共识度进行加权平均
 */
export async function calculateTopicConsensus(topicId) {
    ...
    userConsensusRecords.forEach(record => {
        if (record.consensusScore !== null) {
            ...
            const avgDepth = discussionPaths.length > 0
                ? discussionPaths.reduce((sum, path) => sum + (path.depth || 0), 0) / discussionPaths.length
                : 1;
            // 权重计算：
 ...
 * 基于所有用户对的共识度聚合结果
 */
export async function updateTopicConsensusSnapshot(topicId) {
    ...
    const allConsensusPoints = new Map();
    const allDisagreementPoints = new Map();
    userConsensusRecords.forEach(record => {
        const consensus = record.consensus || [];
        const disagreements = record.disagreements || [];
 ...
 * 如果不存在，需要实现
 */
async function createConsensusSnapshot(topicId, consensusData) {
    // 检查是否已有快照（保留最近50个）
