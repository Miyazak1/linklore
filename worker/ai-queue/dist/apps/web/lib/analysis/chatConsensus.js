 ...
 * @returns 分析结果
 */
export async function analyzeChatConsensus(roomId) {
    ...
    // 统计参与度
    const { creatorMessageCount, participantMessageCount, creatorAiAdoptionCount, participantAiAdoptionCount, creatorAiSuggestionCount, participantAiSuggestionCount } = calculateParticipationStats(messages, room.creatorId, room.participantId);
    // 生成趋势数据
 ...
 * 分析共识点
 */
async function analyzeConsensusPoints(messages, creatorId, participantId) {
 ...
 * 分析分歧点
 */
async function analyzeDisagreementPoints(messages, creatorId, participantId) {
    ...
    for (let i = 0; i < messages.length; i++) {
        for (let j = i + 1; j < messages.length; j++) {
            ...
            const hasReference = msg2.references?.some((ref) => ref.referencedMessage.id === msg1.id);
            if (hasReference) {
                // 计算语义相似度
                try {
                    const similarity = await calculateSemanticSimilarity(msg1.content, msg2.content);
 ...
 * 计算共识度
 */
function calculateConsensusScore(consensusPoints, totalMessages) {
    if (totalMessages < 2)
        return 0.5;
    // 基于共识点的数量和置信度计算
 ...
 * 计算分歧度
 */
function calculateDivergenceScore(disagreementPoints, totalMessages) {
    if (totalMessages < 2)
        return 0.5;
 ...
 * 计算引用深度
 */
function calculateReferenceDepth(messages) {
    let totalDepth = 0;
    let maxDepth = 0;
 ...
 * 计算AI采纳率
 */
function calculateAiAdoptionRate(messages) {
 ...
 * 统计参与度
 */
function calculateParticipationStats(messages, creatorId, participantId) {
