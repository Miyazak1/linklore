 ...
 * 分析实践记录的深度（Worker任务）
 */
export async function analyzePracticeJob(practiceId) {
    try {
        // 获取实践记录
        ...
        const [analysis, realityCheck] = await Promise.all([
            analyzePracticeDepth(practice.content, practice.structuredData, practice.authorId),
            // 执行现实性验证
            validatePracticeReality(practice.content, practice.structuredData, practice.mediaFiles.length, practice.authorId),
        ]);
        // 获取互动数据
        ...
        // 计算综合分数（包含现实性分数和验证状态）
        const compositeScore = calculateCompositeScore(analysis.depthScore, analysis.densityScore, practice.createdAt, likeCount, commentCount, realityCheck.score, // 现实性分数
        ...
        await prisma.$transaction([
            ...
            prisma.practiceAnalysis.upsert({
                ...
                update: {
                    ...
                    theoryMatches: analysis.theoryMatches,
                    analyzedAt: new Date(),
                },
                create: {
                    practiceId,
            ...
            prisma.practice.update({
                ...
                data: {
                    ...
                    densityScore: analysis.densityScore,
                    compositeScore,
                    realityCheck: realityCheck, // 保存现实性检查结果
                    aiProcessingStatus: {
                        analysis: 'completed',
                    },
                },
